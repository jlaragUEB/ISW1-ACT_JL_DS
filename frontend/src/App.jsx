// ISW1JLDS-7 — Historia: Visualizar tarjetas en tabla
// Dayana — Branch: ISW1JLDS-7
...
const FranquiciaTag = ({ nombre }) => {
  ...
  return (
    <span style={{
      background: colores[nombre] || '#555', color: '#fff',
    ...
    <Modal titulo="Registrar Tarjeta de Crédito" onClose={onClose}>
      <div style={{ marginBottom: 16 }}>
        ...
        <select value={form.clienteId} onChange={set('clienteId')}
          style={{ width: '100%', padding: '9px 12px', border: '1.5px solid #ddd', borderRadius: 8, fontSize: 14, fontFamily: "'IBM Plex Sans', sans-serif", outline: 'none', boxSizing: 'border-box', background: '#fff' }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f0f2f8 0%, #e8ecf5 100%)', fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      <main style={{ padding: '32px 40px' }}>
        ...
        <p style={{ fontSize: 14, color: '#666', margin: '0 0 24px' }}>{tarjetas.length} registro(s) en total</p>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: '#1a1f71', color: '#fff' }}>
                  {['N° Tarjeta', 'Vencimiento', 'Franquicia', 'Estado', 'Cupo Total', 'Cupo Disponible', 'Cupo Utilizado', 'Cliente'].map(h => (
              ...
              <tbody>
                {tarjetas.length === 0 && (
                  <tr><td colSpan={8} style={{ padding: 40, textAlign: 'center', color: '#aaa', fontSize: 14 }}>
